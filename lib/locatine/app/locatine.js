...
async function addOptions() {
  ...
  if ((document.data.uniq.length === 0) && (document.data.usual.length === 0)) {
    document.data = document.current;
  } else {
    const uniq = document.data.uniq.filter(x => document.current.uniq.includes(x));
    const usual = document.data.usual.filter(x => document.current.usual.includes(x));
    document.data = {"uniq": uniq, "usual": usual};
  }
  await populateList();
  document.elementsCount = document.elementsCount +1;
  document.getElementById('results-title').innerHTML = document.elementsCount + " elements were added. You can find them by names below.\nHigher in the list the better";
}

function populateList() {
  document.data.uniq.map(x => addOption(x));
  document.data.usual.map(x => addOption(x));
}
...
async function clearOptions(reload) {
  ...
  if (reload){
    ...
    document.elementsCount = 0;
    document.getElementById('results-title').innerHTML = "Select and add something!"
  }
}
